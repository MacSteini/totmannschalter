...
/* USER CONFIGURATION
 * Set TOTMAN_UI_SETUP_CODE before first setup. Set TOTMAN_UI_CONFIG_FILE to an
 * absolute private path when the script itself lives in a public webroot.
...
const UI_DEFAULT_CONFIG_FILE = __DIR__ . '/.totman-ui.php';
...
function ui_config_file(): string
{
    $envPath = getenv('TOTMAN_UI_CONFIG_FILE');
    if (is_string($envPath) && trim($envPath) !== '') {
        $path = trim($envPath);
        if (!str_starts_with($path, '/') || str_contains($path, "\0") || is_dir($path) || basename($path) === '') {
            throw new RuntimeException('TOTMAN_UI_CONFIG_FILE must point to an absolute private PHP file path.');
        }
        return $path;
    }
    return UI_DEFAULT_CONFIG_FILE;
}
