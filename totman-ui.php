 ...
 * plus the generated .totman-ui.php.
 */

declare(strict_types=1);

/* USER CONFIGURATION
 * Set TOTMAN_UI_SETUP_CODE before first setup. The other values tune session lifetime,
 * brute-force throttling, response delay after failed auth, and backup retention.
 */
const TOTMAN_UI_SETUP_CODE = 'CHANGE_THIS_SETUP_CODE';
...
const UI_CONFIG_FILE = __DIR__ . '/.totman-ui.php';
...
{
    if (!is_file(UI_CONFIG_FILE)) {
        return null;
    }
    $cfg = require UI_CONFIG_FILE;
...
function ui_write_config(array $cfg): void
{
    ...
    atomic_write(UI_CONFIG_FILE, $body);
}
...
function preflight_checks(array $ui, ?array $main, ?array $recips, ?array $state): array
{
    ...
    try {
        ...
        $checks[] = ['error',t('Data directory'),$e->getMessage(),t('.totman-ui.php must contain the correct state_dir; after correction, the page can be reloaded.')];
    }
...
{
    return current_script_name() . '?asset=' . rawurlencode($asset) . '&v=' . pwa_version();
}
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'recover_state_dir') {
            ...
            ensure_setup_state_dir($newStateDir);
        ...
        if ($action === 'save_main') {
            ...
            with_totman_lock($main['config'], function () use ($main, $recips, $recipsBackup, $recipientsData, &$fileDeleteSummary): void {
                delete_download_alias_files($main['config'], $fileDeleteSummary);
                try {
                    write_recipients_config($recips['live_path'], $recipientsData);
                } catch (Throwable $e) {
                    if ($fileDeleteSummary !== []) {
                        $backupPath = $recipsBackup ?? 'none';
                        throw new RuntimeException("Recipients config save failed after deleting files. Backup review required: {$backupPath}. Original error: " . $e->getMessage());
                    }
                    throw $e;
                }
            });
...
function render_main_config(array $main, array $flash): void
{
    ...
    input_readonly('state_dir_display', 'Data Directory', (string)($cfg['state_dir'] ?? ''), 'This path comes from .totman-ui.php. Moving it requires .totman-ui.php changes or setup recovery.');
