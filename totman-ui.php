...
// Generated totman runtime UI candidate. Do not edit this file by hand.
...
final class ProductCandidateAcceptanceReport
{
    /**
     * @param array<string, mixed> $manifest
     * @param array<string, mixed> $writePathResult
     * @param array<string, mixed> $productSmokeResult
     * @param array<string, mixed> $browserFlowResult
     * @param array<string, mixed> $surfaceResult
     * @param array<string, mixed> $errorSurfaceResult
     * @param array<string, mixed> $dangerResult
     * @param array<string, mixed> $fingerprintResult
     */
    public function __construct(
        private readonly array $manifest,
        private readonly array $writePathResult,
        private readonly array $productSmokeResult,
        private readonly array $browserFlowResult = [],
        private readonly array $surfaceResult = [],
        private readonly array $errorSurfaceResult = [],
        private readonly array $dangerResult = [],
        private readonly array $fingerprintResult = [],
    ) {
    }
    ...
    {
        return [
            'bundle_mode' => $this->manifest['runtime_ui_mode'] ?? '',
            'entry_mode' => $this->manifest['entry_mode'] ?? '',
            'source_files' => $this->manifest['source_files'] ?? [],
            'excluded' => $this->manifest['excluded'] ?? [],
            'write_path_result' => $this->writePathResult,
            'product_smoke_result' => $this->productSmokeResult,
            'browser_flow_result' => $this->browserFlowResult,
            'surface_result' => $this->surfaceResult,
            'error_surface_result' => $this->errorSurfaceResult,
            'danger_result' => $this->dangerResult,
            'fingerprint_result' => $this->fingerprintResult,
        ];
    }
...
final class ProductCandidateFingerprint
{
    /**
     * @param array<string, mixed> $manifest
     */
    public function __construct(
        private readonly array $manifest,
        private readonly string $bundleSource,
        private readonly string $generatedAtUtc,
    ) {
    }
    ...
    public function data(): array
    {
        ...
        return [
            ...
            'namespace' => 'Totman\\RuntimeUi',
            'source_file_count' => count($sourceFiles),
            'source_files_sha256' => hash('sha256', implode("\n", $sourceFiles)),
            'excluded' => $excluded,
            'checks' => [
                'runtime_ui_namespace_present' => str_contains($this->bundleSource, 'namespace Totman\\RuntimeUi\\Application;'),
                'refactoring_namespace_absent' => !str_contains($this->bundleSource, 'namespace Totman\\Refactoring\\Application;'),
                'product_runtime_files_excluded' => in_array('product runtime files', $excluded, true),
                'secrets_excluded' => in_array('secrets', $excluded, true),
                'runtime_state_excluded' => in_array('runtime state', $excluded, true),
                'private_ui_config_excluded' => in_array('.totman-ui.php', $excluded, true),
                'generated_configs_excluded' => in_array('generated configs', $excluded, true),
            ],
        ];
    }

    public function containsSensitiveText(string ...$needles): bool
    {
    ...
    {
        $sourceFiles = $this->manifest['source_files'] ?? [];
...
final class ProductIntegrationPlan
{
    /**
     * @return array<string, mixed>
     */
    ...
    {
        return [
            'target' => '../totman/totman-ui.php',
            'source' => 'generated product-candidate bundle',
            'copy_rule' => 'Copy only the reviewed single-file candidate; do not copy Refactoring source, tests, reports, runtime state, logs, secrets, or fixtures.',
            'required_refactoring_checks' => [
                'php tests/smoke.php',
                'php tests/compatibility.php',
                'php tests/bundle.php',
                'php tests/product-bundle.php',
                'php tests/integration-rehearsal.php',
                'php tests/browser-semantics.php',
                'php tests/product-ui-surface-diff.php',
                'php tests/product-integration-plan.php',
                'php tests/product-candidate-runtime-contract.php',
                'php tests/product-mutation-contract.php',
                "find src tests public -name '*.php' -exec php -l {} \\;",
                'php -l tools/build-ui-bundle.php',
            ],
            'required_product_checks_after_copy' => [
                'php -l totman-ui.php',
                "find . -name '*.php' -exec php -l {} \\;",
                'php .codex-project/tools/totman/audit-smoke.php from workspace root, if available',
                'php totman-tick.php check in a configured test runtime',
                'focused browser smoke for setup, login, draft, final save, admin inspection, maintenance previews, and maintenance execution',
                'git diff --check',
            ],
            'documentation_delta' => [
                'README.md',
                ...
                'docs/Web.md',
                'docs/Changelog.md',
                'release archive and packaging notes',
                'Use Refactoring/PRODUCT-INTEGRATION-DOC-DELTA.md as the checklist source.',
            ],
            'deferred' => [
                'final visual design',
                'full UI localisation',
                'Docker or Podman deployment changes',
            ],
            'rollback' => [
                'Before product copy, record the current totman/totman-ui.php blob or commit.',
                'Rollback is restoring the previous single file and rerunning the product checks.',
                'Do not use git reset; preserve unrelated user changes.',
            ],
            'go_no_go' => [
                'Go only if Refactoring tests are green and the generated candidate has no visible prototype/refactoring markers.',
                'No-go if product copy requires changes outside totman/totman-ui.php or product documentation without separate approval.',
                'No-go if maintenance execution bypasses admin session, CSRF, rate limits, current reauthentication, explicit confirmation, or product-parity checks.',
            ],
        ];
    }
    ...
    public function toMarkdown(): string
    {
        $data = $this->data();
        $lines = [
            '# Product UI Integration Plan',
            '',
            'This plan is generated by `Refactoring/` as a handover aid. It is not a product change.',
            '',
            '## Target',
            '',
            '- Target file: `' . $data['target'] . '`',
            '- Source artefact: ' . $data['source'],
            '- Copy rule: ' . $data['copy_rule'],
            '',
            '## Required Refactoring Checks',
            '',
        ];

        $lines = array_merge($lines, $this->list($data['required_refactoring_checks']));
        $lines[] = '';
        $lines[] = '## Required Product Checks After Copy';
        $lines[] = '';
        $lines = array_merge($lines, $this->list($data['required_product_checks_after_copy']));
        $lines[] = '';
        $lines[] = '## Documentation Delta';
        $lines[] = '';
        $lines = array_merge($lines, $this->list($data['documentation_delta']));
        $lines[] = '';
        $lines[] = '## Deferred';
        $lines[] = '';
        $lines = array_merge($lines, $this->list($data['deferred']));
        $lines[] = '';
        $lines[] = '## Rollback';
        $lines[] = '';
        $lines = array_merge($lines, $this->list($data['rollback']));
        $lines[] = '';
        $lines[] = '## Go/No-Go';
        $lines[] = '';
        $lines = array_merge($lines, $this->list($data['go_no_go']));
...
final class ProductUiSurfaceDiff
{
    public function __construct(
        private readonly string $productSource,
        private readonly string $candidateSource,
        private readonly string $candidateHtml,
    ) {
    }
    ...
    public function data(): array
    {
        $product = $this->productSurface();
        $candidate = $this->candidateSurface();

        return [
            'product_surface' => $product,
            'candidate_surface' => $candidate,
            'missing_candidate_categories' => $this->missingCategories($product, $candidate),
            'deferred_categories' => [
                'full configuration editor',
                'theme controls',
                'full localisation',
            ],
            'surface_diff_ready' => $this->isReady($product, $candidate),
        ];
    }
     ...
     */
    private function productSurface(): array
    {
        return [
            'shell' => str_contains($this->productSource, '.card') && str_contains($this->productSource, '.header'),
            'forms' => str_contains($this->productSource, '<form') && str_contains($this->productSource, '<input'),
            'alerts' => str_contains($this->productSource, 'notice ok') || str_contains($this->productSource, 'notice error'),
            'preflight' => str_contains($this->productSource, 'Preflight') || str_contains($this->productSource, 'preflight'),
            'logs' => str_contains($this->productSource, 'Log entries') || str_contains($this->productSource, 'log-window'),
            'admin_access' => str_contains($this->productSource, 'Create access') || str_contains($this->productSource, 'Sign in'),
            'danger_zone' => str_contains($this->productSource, 'Danger Zone') || str_contains($this->productSource, 'danger'),
        ];
    }
     ...
     * @return array<string, bool>
     */
    private function candidateSurface(): array
    {
        return [
            'shell' => str_contains($this->candidateHtml, 'class="ui-shell mode-product-candidate"')
                && str_contains($this->candidateHtml, 'data-surface="product-candidate"'),
            'forms' => str_contains($this->candidateHtml, '<form') && str_contains($this->candidateHtml, '<input'),
            'alerts' => str_contains($this->candidateHtml, 'role="status"') || str_contains($this->candidateHtml, 'role="alert"'),
            'preflight' => str_contains($this->candidateSource, 'Preflight') || str_contains($this->candidateHtml, 'Preflight'),
            'logs' => str_contains($this->candidateSource, 'Log tail') || str_contains($this->candidateHtml, 'Log tail'),
            'admin_access' => str_contains($this->candidateHtml, 'data-section="admin-access"'),
            'danger_zone' => str_contains($this->candidateSource, 'preview_runtime_reset')
                && str_contains($this->candidateSource, 'confirm_admin_command'),
        ];
    }

    /**
     * @param array<string, bool> $product
     * @param array<string, bool> $candidate
     * @return list<string>
     */
    private function missingCategories(array $product, array $candidate): array
    {
        $missing = [];
        foreach ($product as $category => $present) {
            if ($present && !($candidate[$category] ?? false)) {
                $missing[] = $category;
            }
        }
     ...
     * @param array<string, bool> $candidate
     */
    private function isReady(array $product, array $candidate): bool
    {
        return $this->missingCategories($product, $candidate) === [];
    }
}
...
final class PrototypeApplicationFactory
{
    ...
    public function __construct(
        ...
        $generatedByLine = $runtimeUiMode === RuntimeUiMode::PRODUCT_CANDIDATE
...
final class RuntimeUiTextCatalog
{
    ...
    public function all(): array
    {
        ...
        if ($this->runtimeUiMode === RuntimeUiMode::PRODUCT_CANDIDATE) {
    ...
    public function productCandidate(): bool
    {
        return $this->runtimeUiMode === RuntimeUiMode::PRODUCT_CANDIDATE;
    }
...
final class ProductRuntimeContextAdapter
{
    ...
    public function fromArrays(array $server, array $post, array $env, string $defaultStateDir): PrototypeEnvironment
    {
        return $this->environmentFactory->fromArrays(
            ...
            $env,
            $defaultStateDir,
            RuntimeUiMode::PRODUCT_CANDIDATE
        );
    }
}
...
final class PrototypeEnvironmentFactory
{
    ...
    private function stateDir(array $query, array $env, string $defaultStateDir, string $runtimeUiMode): array
    {
        ...
        if ($runtimeUiMode === RuntimeUiMode::PRODUCT_CANDIDATE) {
        ...
        $envStateDir = $this->string($env['TOTMAN_REFACTOR_STATE_DIR'] ?? null);
        if ($envStateDir !== '') {
    ...
    private function contextName(array $query, array $env, string $runtimeUiMode): string
    {
        ...
        return $this->string($env['TOTMAN_REFACTOR_CONTEXT'] ?? null) ?: 'classic';
    }
    ...
    private function setupCode(array $env, string $runtimeUiMode): string
    {
        ...
        return $this->string($env['TOTMAN_REFACTOR_SETUP_CODE'] ?? null);
    }
...
<div class="ui-actions">' . $this->renderActions($view) . '</div>
    ...
    {
        $mode = $this->text->productCandidate() ? 'product-candidate' : 'prototype';
...
.mode-product-candidate .ui-main{width:min(1120px,100%);margin:0 auto;padding:clamp(16px,3vw,40px)}
.mode-product-candidate .ui-header{margin:0 0 1rem}
.mode-product-candidate .ui-product{margin:0 0 .25rem;color:var(--ui-accent);font-weight:800;letter-spacing:.08em;text-transform:lowercase}
.mode-product-candidate h1{margin:0;font-size:clamp(1.6rem,1.1rem + 2vw,2.6rem);line-height:1.1}
.mode-product-candidate h2{margin:0 0 .8rem;font-size:1.12rem}
.mode-product-candidate h3{margin:1rem 0 .5rem;font-size:1rem}
.mode-product-candidate .ui-panel{background:var(--ui-surface);border:1px solid var(--ui-border);border-radius:16px;padding:clamp(16px,2vw,24px);margin:0 0 1rem;box-shadow:0 10px 32px #0f172a12}
.mode-product-candidate .ui-form{display:grid;gap:1rem}
.mode-product-candidate .ui-wizard-form fieldset{border:1px solid var(--ui-border);border-radius:14px;padding:1rem;margin:0;background:#fff}
.mode-product-candidate .ui-wizard-form legend{font-weight:800;padding:0 .35rem}
.mode-product-candidate .ui-field{display:grid;gap:.35rem;margin:.75rem 0}
.mode-product-candidate .ui-summary{display:grid;grid-template-columns:minmax(9rem,14rem) 1fr;gap:.45rem 1rem}
.mode-product-candidate dt{font-weight:700;color:var(--ui-text)}
.mode-product-candidate dd{margin:0;color:var(--ui-muted)}
.mode-product-candidate label{font-weight:650}
.mode-product-candidate input,.mode-product-candidate textarea{width:100%;max-width:44rem;border:1px solid var(--ui-border);border-radius:10px;padding:.65rem .75rem;font:inherit;background:#fff;color:var(--ui-text)}
.mode-product-candidate textarea{min-height:7rem}
.mode-product-candidate input[type=checkbox]{width:auto}
.mode-product-candidate button{appearance:none;border:1px solid var(--ui-accent);background:var(--ui-accent);color:#fff;font:inherit;font-weight:700;border-radius:10px;padding:.65rem .95rem;min-height:42px;cursor:pointer}
.mode-product-candidate button:hover{background:var(--ui-accent-hover);border-color:var(--ui-accent-hover)}
.mode-product-candidate button:focus-visible,.mode-product-candidate input:focus-visible,.mode-product-candidate textarea:focus-visible{outline:3px solid #99f6e4;outline-offset:2px}
.mode-product-candidate .ui-actions{display:flex;gap:.6rem;flex-wrap:wrap;margin-top:1rem}
.mode-product-candidate .ui-alert{border:1px solid #fecaca;background:#fef2f2;color:var(--ui-danger);border-radius:12px;padding:1rem 1.2rem}
.mode-product-candidate .ui-status{border:1px solid #bbf7d0;background:#f0fdf4;color:var(--ui-ok);border-radius:12px;padding:.8rem 1rem}
.mode-product-candidate pre{max-height:22rem;overflow:auto;background:#111827;color:#f9fafb;border-radius:12px;padding:1rem}
.mode-product-candidate code{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono","Courier New",monospace;background:#f3f4f6;border:1px solid #e5e7eb;border-radius:8px;padding:.12em .42em}
@media (max-width:640px){.mode-product-candidate .ui-main{padding:12px}.mode-product-candidate .ui-panel{border-radius:12px;padding:16px}.mode-product-candidate .ui-summary{grid-template-columns:1fr}.mode-product-candidate button{width:100%}}
</style>';
    }
...
<button type="submit" name="action" value="admin_command">' . $this->e($this->text->get('danger.execute')) . ': ' . $this->e($label) . '</button>
    ...
    {
        return $this->text->productCandidate() ? $this->text->get('summary.state_dir_hidden') : $view->stateDir();
    }
...
final class RuntimeUiMode
{
    ...
    public const PRODUCT_CANDIDATE = 'product-candidate';
    ...
    {
        return $mode === self::PRODUCT_CANDIDATE ? self::PRODUCT_CANDIDATE : self::PROTOTYPE;
    }
}
...
final class BundleManifest
{
    ...
    public static function data(): array
    {
        return
array (
  'entry_mode' => 'product candidate bundle',
  'runtime_ui_mode' => 'product-candidate',
  ...
  array (
    ...
    38 => 'src/Application/ProductCandidateAcceptanceReport.php',
    39 => 'src/Application/ProductCandidateFingerprint.php',
    40 => 'src/Application/ProductIntegrationPlan.php',
    41 => 'src/Application/ProductUiSurfaceDiff.php',
    42 => 'src/Application/PrototypeApplicationFactory.php',
    43 => 'src/Application/PrototypePageApplicationService.php',
    44 => 'src/Application/PrototypePageResult.php',
    45 => 'src/Application/RecipientConfigDraft.php',
    46 => 'src/Application/RecipientConfigDraftBuilder.php',
    47 => 'src/Application/RuntimeLogReader.php',
    48 => 'src/Application/RuntimeLogTail.php',
    49 => 'src/Application/RuntimeStateFileStatus.php',
    50 => 'src/Application/RuntimeSummary.php',
    51 => 'src/Application/RuntimeSummaryReader.php',
    52 => 'src/Application/RuntimeUiTextCatalog.php',
    53 => 'src/Config/ConfigCoverageResult.php',
    54 => 'src/Config/ConfigDiscovery.php',
    55 => 'src/Config/ConfigFileStatus.php',
    56 => 'src/Config/DiscoveryIssue.php',
    57 => 'src/Config/DiscoveryResult.php',
    58 => 'src/Config/ImportedField.php',
    59 => 'src/Config/MainConfigCoverage.php',
    60 => 'src/Config/MainConfigImport.php',
    61 => 'src/Config/MainConfigImporter.php',
    62 => 'src/Config/MainConfigWriter.php',
    63 => 'src/Config/RecipientConfigCoverage.php',
    64 => 'src/Config/RecipientConfigImport.php',
    65 => 'src/Config/RecipientConfigImporter.php',
    66 => 'src/Config/RecipientConfigWriter.php',
    67 => 'src/Contracts/RuntimeFileNames.php',
    68 => 'src/Deployment/DeploymentCapabilities.php',
    69 => 'src/Deployment/DeploymentContext.php',
    70 => 'src/Http/FirstRunRequest.php',
    71 => 'src/Http/FirstRunRequestMapper.php',
    72 => 'src/Http/ProductRuntimeContextAdapter.php',
    73 => 'src/Http/PrototypeController.php',
    74 => 'src/Http/PrototypeEnvironment.php',
    75 => 'src/Http/PrototypeEnvironmentFactory.php',
    76 => 'src/Http/PrototypeRenderer.php',
    77 => 'src/Http/RuntimeUiMode.php',
    78 => 'src/Preflight/FirstRunPreflight.php',
    79 => 'src/Preflight/PreflightCheck.php',
    80 => 'src/Preflight/PreflightResult.php',
    81 => 'src/Security/AdminAuthInput.php',
    82 => 'src/Security/AdminAuthService.php',
    83 => 'src/Security/AdminCredential.php',
    84 => 'src/Security/AdminReauthPolicy.php',
    85 => 'src/Security/AdminSessionLoadResult.php',
    86 => 'src/Security/AdminSessionState.php',
    87 => 'src/Security/AdminSessionStore.php',
    88 => 'src/Security/CsrfTokens.php',
    89 => 'src/Security/HmacSecretGenerator.php',
    90 => 'src/Security/InMemoryRateLimiter.php',
    91 => 'src/Security/PrototypeCsrfPolicy.php',
    92 => 'src/Security/PrototypeRateLimitPolicy.php',
    93 => 'src/Security/PrototypeSaveIntentPolicy.php',
    94 => 'src/Security/SecretRedactor.php',
    95 => 'src/Security/SessionSecurity.php',
    96 => 'src/Security/SetupAccessPolicy.php',
    97 => 'src/Security/SetupAccessResult.php',
    98 => 'src/Security/SetupCodeVerifier.php',
    99 => 'src/Security/SetupSessionLoadResult.php',
    100 => 'src/Security/SetupSessionState.php',
    101 => 'src/Security/SetupSessionStore.php',
    102 => 'src/Security/UiPrivateConfig.php',
    103 => 'src/Security/UiPrivateConfigLoadResult.php',
    104 => 'src/Security/UiPrivateConfigStore.php',
    105 => 'src/Setup/FirstRunFlow.php',
    106 => 'src/Setup/FirstRunOrchestrator.php',
    107 => 'src/Setup/FirstRunPlanner.php',
    108 => 'src/Setup/FirstRunStepCatalog.php',
  ),
  'excluded' =>
  array (
...
final class PrototypeBundle
{
    ...
    {
        $runtimeUiMode ='product-candidate';
        $env = [
            'TOTMAN_REFACTOR_CONTEXT' => getenv('TOTMAN_REFACTOR_CONTEXT') ?: '',
            'TOTMAN_REFACTOR_SETUP_CODE' => getenv('TOTMAN_REFACTOR_SETUP_CODE') ?: '',
            'TOTMAN_REFACTOR_STATE_DIR' => getenv('TOTMAN_REFACTOR_STATE_DIR') ?: '',
        ...
        $environment = $runtimeUiMode === 'product-candidate'
...
if (!defined('TOTMAN_REFACTOR_BUNDLE_NO_RUN')) {
    PrototypeBundle::run();
}
