...
3. Set required config in `/var/lib/totman/totman.inc.php` or, if you intentionally use the `.dist.php` file as your runtime config, in `/var/lib/totman/totman.inc.dist.php`:
	...
	- To use the optional Web UI, deploy `totman-ui.php` into the webroot, set `TOTMAN_UI_SETUP_CODE` server-side for first-run setup, and use `web_ui_enabled` to control browser administration after setup
